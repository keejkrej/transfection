...
# Defaults for `delivery expression fit` (defined here; always passed explicitly from this script).
...
function Invoke-DeliveryExpression {
    param([string[]]$ExprArgs)
    Push-Location $RepoRoot
    try {
        Write-Host "`n>> $UvExe run delivery expression $($ExprArgs -join ' ')`n" -ForegroundColor Cyan
        $allArgs = @("run", "delivery", "expression") + $ExprArgs
...
Write-Host ("`n" + 'expression timeseries & fit - set --jobs and (for fit) --max-onset-minutes (defaults from this script):') -ForegroundColor DarkGray
...
if ($runTimeseries) {
    ...
    $code = Invoke-DeliveryExpression (@(
        "timeseries", $workspace,
        "--sample", $slidePath,
        "--jobs", "$fitJobs"
    ) + $correctionArgs)
    Exit-IfFailed $code "expression timeseries"
}
...
Exit-IfFailed $code "expression plot-timeseries"

$code = Invoke-DeliveryExpression @(
    "auc", $workspace,
    "--interval", $intervalStr
)
Exit-IfFailed $code "expression auc"
...
$code = Invoke-DeliveryExpression @("plot-auc", $aucCsv)
Exit-IfFailed $code "expression plot-auc"
...
Exit-IfFailed $code "expression fit"
...
Exit-IfFailed $code "expression plot-fit"
