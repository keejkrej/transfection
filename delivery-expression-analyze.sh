#!/usr/bin/env bash
# Interactive pipeline for delivery expression: timeseries, plots, AUC, fit.
# Dev: from repo root, run: bash scripts/delivery-expression-analyze.sh
# Prod (delivery.zip): run from extracted bundle root after install.sh
...
if [[ -d "$SCRIPT_DIR/../apps/delivery" ]]; then
  ...
  echo "Run from repo (scripts/delivery-expression-analyze.sh) or extracted delivery bundle root." >&2
  exit 1
fi
...
invoke_delivery_expression() {
  local -a args=("$@")
  echo "" >&2
  echo ">> $UV_EXE run delivery expression ${args[*]}" >&2
  echo "" >&2
  (cd "$REPO_ROOT" && exec "$UV_EXE" run delivery expression "${args[@]}")
}
...
delivery expression analyze
---------------------------
Runs in order: timeseries (optional) -> plot-timeseries -> auc -> plot-auc -> fit -> plot-fit
expression timeseries and fit share --jobs; fit also receives --max-onset-minutes (defaults from this script, Enter to accept).
...
echo "" >&2
echo "expression timeseries & fit — set --jobs and (for fit) --max-onset-minutes (defaults from this script):" >&2
...
if [[ "$run_timeseries" -eq 1 ]]; then
  ...
  code=$?
  set -e
  exit_if_failed "$code" "expression timeseries"
fi
...
set +e
invoke_delivery_expression plot-timeseries "$ts_dir"
code=$?
set -e
exit_if_failed "$code" "expression plot-timeseries"

set +e
invoke_delivery_expression auc "$workspace" --interval "$interval"
code=$?
set -e
exit_if_failed "$code" "expression auc"
...
set +e
invoke_delivery_expression plot-auc "$auc_csv"
code=$?
set -e
exit_if_failed "$code" "expression plot-auc"

set +e
invoke_delivery_expression fit "$workspace" \
  --interval "$interval" \
  --jobs "$fit_jobs" \
  --max-onset-minutes "$fit_max_onset"
code=$?
set -e
exit_if_failed "$code" "expression fit"
...
set +e
invoke_delivery_expression plot-fit "$fit_csv" --interval "$interval"
code=$?
set -e
exit_if_failed "$code" "expression plot-fit"
